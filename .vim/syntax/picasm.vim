...
" OpCodes...
syn keyword mspOpcode  ANDWF CLRF CLRW COMF DECF DECFSZ INCF INCFSZ IORWF MOVF MOVWF NOP RLF RRF SUBWF SWAPF XORWF BCF BSF BTFSC BTFSS ADDLW ANDLW CALL CLRWDT GOTO IORLW MOVLW RETFIE RETLW RETURN SLEEP SUBLW XORLW


" Directives
syn keyword mspDirective CONSTANT DATA DS DB DE DT DW ELSE END ENDC
syn keyword mspDirective ENDM ENDW ENDIF EQU ERRORLEVEL EXITM EXPAND
syn keyword mspDirective EXTERN FILL GLOBAL IDATA __IDLOCS 
syn keyword mspDirective IF INCLUDE LIST LOCAL MACRO __MAXRAM MESSG NOEXPAND
...
syn match mspPreproc   "#\s*UNDEF"
syn match mspPreproc   "#\s*UNDEFINE"
syn match mspPreproc   "#\s*INCLUDE"
syn match mspPreproc   "#\s*DEFINE"
syn match mspPreproc   "#\s*ELIF"
syn match mspPreproc   "#\s*IF"
syn match mspPreproc   "#\s*IFDEF"
syn match mspPreproc   "#\s*IFNDEF"
syn match mspPreproc   "#\s*ENDIF"
syn match mspPreproc   "#\s*ERROR"
syn match mspPreproc   "#\s*ELSE"
...
if version >= 508 || !exists("did_pic16f84_syntax_inits")
  ...
  HiLink mspDirective          Label
  HiLink mspPreproc            PreCondit
  HiLink mspLabel              Special
  HiLink mspString             String

  HiLink mspOpcode            Structure
